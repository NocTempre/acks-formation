# Exploration Formations (acks-formation)
...
- `scripts/` — ESM runtime, entry `scripts/module.mjs`; `templates/` — .hbs;
  `styles/`; `lang/en.json` — flat i18n keys prefixed `ACKS-FORMATION.`
...
- `tools/` — dev harness. `build-packs.mjs` and `validate.mjs` are **synced
  ...
  module-owned.
- Canonical ACKS II rules extract: `C:\Proj\acks-rules\acks-formation\RULES.md`
...
- Foundry dev install (junction, not copy):
  `New-Item -ItemType Junction -Path "$env:LOCALAPPDATA\FoundryVTT\Data\modules\acks-formation" -Target "C:\Proj\acks-formation"`
## Live testing
...
5. Confirm publication with BOUNDED polls — **never `gh run watch`, it hangs**:
   ...
   verify `https://github.com/NocTempre/acks-formation/releases/latest/download/module.json`
...
- Namespacing (validate-enforced): globals/custom hooks/HB helpers start with
  ...
  `flags["acks-formation"].idPrefix` key; lang keys with `ACKS-FORMATION.`;
  CSS classes with `acks-formation-`.
- Design doctrine: **reuse → extend → enhance → invent** — reuse core system
  documents; extend only via `flags["acks-formation"]`; enhance with alternate
